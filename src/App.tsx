...
const AppContent: React.FC = () => {
  ...
  /**
   * @desc Load shops from Firestore on mount
   */
  useEffect(() => {
    const loadShops = async () => {
      try {
        ...
        if (targetCity && targetCity !== 'All') {
          setShops(filterShopsByGeolocation(shopsWithTiers, { city: targetCity }));
        } else {
          setShops(shopsWithTiers);
        }
      } catch (error) {
  ...
  const handleCheckout = async (
    ...
    const newOrder: Order = {
      id: `KLY-${Math.random().toString(36).substring(2, 12).toUpperCase()}`,
  ...
  const renderView = () => {
    ...
    switch (view) {
      ...
      case 'shopOnboarding':
        return <ShopOnboarding setView={navigate} showToast={showToast} />;

      case 'emailVerification':
        return <EmailVerification setView={navigate} email="" onResendEmail={() => { }} />;
