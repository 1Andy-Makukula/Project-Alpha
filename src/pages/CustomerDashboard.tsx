...
import {
    ...
    ChatBubbleIcon, HistoryIcon, HeartIcon, CarIcon
        ...
        switch (activeTab) {
            case 'dashboard':
                ...
                const getStatusBadge = (order: Order) => {
                    if (order.status === 'collected') return { label: 'Completed', color: 'bg-green-100 text-green-700' };
                    if (order.status === 'dispatched') return { label: 'En Route', color: 'bg-blue-100 text-blue-700' };
                    if (order.status === 'ready_for_dispatch') return { label: 'Processing', color: 'bg-yellow-100 text-yellow-700' };
                    return { label: order.deliveryMethod === 'delivery' ? 'Pending Dispatch' : 'Awaiting Pickup', color: 'bg-orange-100 text-orange-700' };
                };
                                ...
                                {recentOrders.length > 0 ? recentOrders.map(order => (
                                    <div key={order.id} className="bg-white p-5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                                        ...
                                        <div className="text-left sm:text-right w-full sm:w-auto flex sm:flex-col items-center sm:items-end justify-between sm:justify-center">
                                            ...
                                            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full mt-1 ${getStatusBadge(order).color}`}>
                                    ...
                                    <div key={order.id} className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100 transition-all hover:shadow-lg">
                                        <div className="p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                                            ...
                                            {order.status === 'dispatched' && order.driverDetails && (
                                                <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 flex items-center gap-3 w-full lg:w-auto mt-4 lg:mt-0">
                                                    <div className="bg-white p-2 rounded-full shadow-sm">
                                                        <CarIcon className="w-5 h-5 text-blue-600" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs font-bold text-blue-800 uppercase tracking-wider">Driver Assigned</p>
                                                        <p className="font-bold text-gray-900">{order.driverDetails.carModel} • {order.driverDetails.plateNumber}</p>
                                                        <p className="text-xs text-gray-600">{order.driverDetails.name} • {order.driverDetails.phone}</p>
                                                    </div>
                                                </div>
                                            )}
                                            <div className="flex flex-col items-end space-y-3 w-full lg:w-auto">
                                                <div className={`px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1 ${order.status === 'dispatched' ? 'bg-blue-50 text-blue-700' :
                                                    order.status === 'ready_for_dispatch' ? 'bg-yellow-50 text-yellow-700' :
                                                        'bg-orange-50 text-orange-700'
                                                    }`}>
                                                    <ClockIcon className="w-3.5 h-3.5" />
                                                    <span>
                                                        {order.status === 'dispatched' ? 'Driver En Route' :
                                                            order.status === 'ready_for_dispatch' ? 'Processing Delivery' :
                                                                order.deliveryMethod === 'delivery' ? 'Pending Dispatch' : 'Awaiting Collection'}
                                                    </span>
                                                </div>
                                ...
                                <div className="animate-fade-in">
                                    ...
                                    <form className="space-y-6 max-w-lg" onSubmit={(e) => { e.preventDefault(); showToast("Profile updated", 'success'); }}>
                                        <div>
                                            ...
                                            <input id="fullName" type="text" defaultValue={currentUser?.displayName || "Ada Lovelace"} className="mt-1 block w-full bg-kithly-light border-transparent rounded-lg py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-kithly-accent transition-shadow" />
                                        </div>
                                        <div>
                                            <label htmlFor="emailAddress" className="block text-sm font-medium text-gray-700">Email Address</label>
                                            <input id="emailAddress" type="email" defaultValue={currentUser?.email || "ada.lovelace@example.com"} className="mt-1 block w-full bg-kithly-light border-transparent rounded-lg py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-kithly-accent transition-shadow" />
                                        </div>
                                        <div className="pt-2">
