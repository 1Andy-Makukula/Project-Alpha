...
const ShopPortal: React.FC<ShopPortalProps> = ({ setView, orders, onMarkAsCollected, showToast, walletStats, notifications: incomingNotifications, onUpdateOrder }) => {
    // Get db from services context
    const { db } = useServices();

    // Get auth context for current user
    const { userProfile, currentUser } = useAuth();

    // Shop data state - loaded from Firestore
    ...
    // Load shop data from Firestore based on logged-in user
    useEffect(() => {
        const loadShopData = async () => {
            ...
            try {
                // First try to get shop by owner ID from user profile
                const shopId = userProfile?.shopId;
                let shop = null;

                if (shopId) {
                    shop = await firestoreShopsService.getById(shopId);
                } else {
                    // Fallback: get shops by owner
                    const shops = await shopService.getByOwner(currentUser.uid);
                    shop = shops[0] || null;
                }

                if (shop) {
                    setCurrentShop(shop);
                    // Sync shop profile state with loaded data
                    setShopProfile({
                        name: shop.name || '',
                        description: shop.description || '',
                        profilePic: shop.profilePic || '',
                        coverPhoto: shop.coverImg || '',
        ...
        loadShopData();
    }, [currentUser?.uid, userProfile?.shopId]);
    ...
    const handleReadyForPickup = (orderId: string) => {
        ...
        // For pickup-only MVP, just notify that order is ready
        showToast("Order is ready for customer pickup!", 'success');
    };

    // Removed dispatch modal handlers - MVP is pickup-only
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (!currentShop?.id) {
            showToast("No shop to update", 'error');
            return;
        }
        ...
        try {
            await firestoreShopsService.update(String(currentShop.id), {
                ...
                profilePic: shopProfile.profilePic || profilePicPreview || '',
                coverImg: shopProfile.coverPhoto || coverPhotoPreview || '',
                openingHours: `${shopProfile.openTime} - ${shopProfile.closeTime}`,
                                        ...
                                        {activeOrderTab === 'requests' ? (
                                            ...
                                            <div className="relative">
                                                ...
                                                <OrderCard
                                                    order={order}
                                                    onMarkAsCollected={() => {
                                                        // Pickup-only: navigate to scan page for verification
                                                        setActiveMenu('scan');
    ...
    return (
        <div className="flex min-h-screen bg-kithly-background pb-20 lg:pb-0">
            ...
            <main className={`flex-1 p-4 md:p-8 overflow-y-auto h-screen transition-all duration-300 lg:ml-${isCollapsed ? '20' : '64'} w-full`}>
                ...
                {/* Dispatch modal removed - MVP is pickup-only */}
            </main>
        </div>
    );
};
