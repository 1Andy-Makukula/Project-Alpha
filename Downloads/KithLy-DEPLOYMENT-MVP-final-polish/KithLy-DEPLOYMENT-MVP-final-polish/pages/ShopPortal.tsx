...
const ShopPortal: React.FC<ShopPortalProps> = ({ setView, orders, onMarkAsCollected, showToast, walletStats }) => {
    ...
    const [activeOrderTab, setActiveOrderTab] = useState<'pickup' | 'history'>('pickup');
    ...
    const filteredOrders = useMemo(() => {
        ...
        return orders
            ...
            .sort((a, b) => new Date(b.collectedOn!).getTime() - new Date(a.collectedOn!).getTime());
    ...
    const handleVerification = (code: string, method: 'scan' | 'manual') => {
        const cleanInput = code.trim().toUpperCase();
        const foundOrder = orders.find(o => o.collectionCode.toUpperCase() === cleanInput && o.status === 'paid');
        ...
        if (foundOrder) {
            ...
            if (method === 'manual') {
               showToast("Invalid Code. Must be KLY + 7 chars.", 'error');
            } else {
               setScanStatus('error');
               setTimeout(() => setScanStatus('idle'), 3000);
            }
    ...
    const renderContent = () => {
        switch (activeMenu) {
            ...
            case 'orders': 
                 const orderTabs = [
                    { id: 'pickup', name: 'New Orders', icon: <PackageIcon className="w-5 h-5"/> },
                    { id: 'history', name: 'Order History', icon: <HistoryIcon className="w-5 h-5"/> },
                ];
                return (
                    <div className="animate-fade-in">
                                    ...
                                    {orderTabs.map(tab => (
                                        ...
                                        >
                                            ...
                                            <span className={`ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium ${activeOrderTab === tab.id ? 'bg-kithly-primary/10 text-kithly-primary' : 'bg-gray-100 text-gray-600'}`}>
                                                {tab.id === 'pickup' ? orders.filter(o => o.status === 'paid').length : orders.filter(o => o.status === 'collected').length}
                                            </span>
                                        </button>
                                    ))}
                                </nav>
    ...
    return (
        <div className="flex min-h-screen bg-kithly-background pb-20 lg:pb-0">
            ...
            <main className={`flex-1 p-4 md:p-8 overflow-y-auto h-screen transition-all duration-300 lg:ml-${isCollapsed ? '20' : '64'} w-full`}>
                ...
                <div className="mb-4">
                    <AnimatedBackButton onClick={() => setView('landing')} label="Back" />
                </div>
